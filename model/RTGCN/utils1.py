...
def wl_iteration(G, labels):
    ...
    for node in G.nodes():
        ...
        # 将当前节点的标签与邻居标签连接
        new_label = (labels[node], tuple(neighbor_labels))
        # 将新标签进行哈希
        new_labels[node] = hash(new_label)
    return new_labels


def wl_role_discovery(G, num_roles, max_iter=5):
    """基于WL算法的角色发现"""
    # 初始化标签
    labels = initialize_labels(G)

    # 迭代多轮 WL 聚合
    for _ in range(max_iter):
        labels = wl_iteration(G, labels)

    # 将标签转化为特征矩阵
    label_features = [[label] for label in labels.values()]

    # 使用聚类算法（如 KMeans）进行角色划分
    kmeans = KMeans(n_clusters=num_roles)
    roles = kmeans.fit_predict(label_features)

    role_node=defaultdict(list)
    # 返回每个节点的角色
    for index,role in enumerate(roles):
        role_node[role].append(index)
    return role_node


# 使用示例
# G = nx.karate_club_graph()  # 使用空手道俱乐部图
# roles = wl_role_discovery(G, num_roles=3)
#
# print(roles)