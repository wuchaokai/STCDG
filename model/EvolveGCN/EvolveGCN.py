...
from model.EvolveGCN.GCNConv import GCNConv
...
class EvolveGCN(nn.Module):
    def __init__(self, nfeat,nhid,out_feat,egcn_type,args,node_num):
        super(EvolveGCN, self).__init__()
        self.input_dim = nfeat
        # self.hidden_dim = 2 * args.nhid
        # self.output_dim = args.nhid
        self.hidden_dim = nhid
        self.output_dim = out_feat
        self.egcn_type = egcn_type

        self.feat = Parameter((torch.ones(node_num, nfeat)).to(args.device), requires_grad=True)

        self.GRCU_layers1 = GRCU(self.input_dim, self.hidden_dim, egcn_type=egcn_type)
        self.GRCU_layers2 = GRCU(self.hidden_dim, self.output_dim, egcn_type=egcn_type)
    ...
    def forward(self, graphs):
        ...
        nodes_list = []
        for g in graphs:
            a_list.append(g.edge_index)
            nodes_list.append(self.feat * g.x)
