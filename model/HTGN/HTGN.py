...
from model.HTGN.hgcn.layers.hyplayers import HGCNConv, HypGRU, HGATConv
from model.HTGN.hgcn.manifolds import PoincareBall
from model.HTGN.BaseModel import BaseModel
...
class HTGN(BaseModel):
    ...
    def weighted_hiddens(self, hidden_window):
        ...
        qk=torch.matmul(hidden_window, self.Q)
        qk = torch.where(torch.isnan(qk), 0, qk)
        e = torch.matmul(torch.tanh(qk), self.r)
