...
import os
from utils.calculate_sim2 import generateStructWeight as generateStructWeight2, generateSimMatrix_Pagerank,caculateTimeSim,generateTimeWeight,complete_graph_with_all_nodes,generateSimMatrix
...
if __name__ == '__main__':
    ...
    if args.gen_sim:
        ...
        for step in range(args.time_steps):
            g=graphs[step]
            path='data/{}/subgraph2/{}/'.format(args.dataset,str(step))
            if args.sim_type =='dtw':
                generateSimMatrix(g,path,node_num,select_rate=1,match_rate=1)
            elif args.sim_type=='pagerank':
    ...
    if args.model=='EvolveGCN':
        ...
        model = SpikeNet(feats[0].shape[1], args.nout, alpha=Spike_args.alpha,
                         ...
                         aggr=Spike_args.aggr, sizes=Spike_args.sizes, surrogate=Spike_args.surrogate,
    ...
    for epoch in range(args.epochs):
        ...
        for idx, feed_dict in enumerate(dataloader):
            ...
            if args.model == 'EvolveGCN':
                ...
                if args.compensate:
                    emb_list=[model[0](nodes,feed_dict['graphs'][:t+1]) for t in range(args.time_steps-2)]
                    emb_list=model[1](emb_list)
        ...
        if test_roc_score > best_auc_test:
    ...
    #     else:
    #         torch.save(best_emb, 'data/{}/spatialTimeCompensate/embed_{}{}.pt'.format(args.dataset,args.model,int(time.time())))
    # else:
    #     torch.save(best_emb, 'data/{}/NoCompensate/embed_{}{}.pt'.format(args.dataset, args.model,int(time.time())))
    new_data = {
